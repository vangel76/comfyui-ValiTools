...
- **Output modes**: a dropdown in the editor toolbar selects `normal`, `H3 t2v`, `H3 r2v` or `Seedance 2.0`; it writes a `/# mode: ... #/` directive into the prompt (you can also type it by hand). Seedance mode keeps `{...}` literal unless it holds a `|` or a `::` weight or is followed by `==<name>`, so dialogue braces survive into the output; H3 mode protects the speech/structure tags from variable substitution. All other syntax is unaffected, and a prompt without a directive behaves exactly as before
