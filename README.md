...
- **Output modes**: a dropdown in the editor toolbar selects `normal`, `H3 t2v`, `H3 r2v` or `Seedance` (2.0 and 2.5, same rules); it writes a `/# mode: ... #/` directive into the prompt (you can also type it by hand). Seedance mode keeps `{...}` literal unless it holds a `|` or a `::` weight, is followed by `==<name>`, or sits behind a switcher guard (`<name>==value::{...}`), so dialogue braces survive into the output while switchers keep working; H3 mode protects the speech/structure tags from variable substitution. All other syntax is unaffected, and a prompt without a directive behaves exactly as before
...
- v1.19.0
  ...
  - VSmartPrompt: the **output cleanup switches are visible again** - `single_line_output`, `remove_whitespaces` and `remove_empty_tags` were still hidden and forced on by the editor script, so v1.17's backend fix could not be used. Note `remove_empty_tags` also collapses `..`, which turns `...` into `.`; switch it off to keep ellipses in dialogue
  - VSmartPrompt: **ellipses survive cleanup** - `...` (and longer dot runs) are dialogue punctuation and are no longer collapsed to `.` by `remove_empty_tags`. The switch now defaults to **off** for new nodes; existing nodes keep their saved value
  - VSmartPrompt: the mode dropdown entry is now just `Seedance` - 2.0 and 2.5 need the same node behaviour; `seedance 2.5` / `sd25` are accepted directive spellings too
  - VSmartPrompt: the **syntax reference no longer switches itself into Seedance mode** - it quotes `/# mode: seedance #/` inside a comment line, and the unanchored directive match picked that up, so the demo prompt resolved with Seedance brace rules and leaked its switcher guards (`bedroom!=kitchen::{...}`) into the output. The directive now has to open its line; a directive written after other text on the same line is ignored
  - VSmartPrompt: in Seedance mode a one-choice block that carries its assignment inside (`{cake==!<act>}`) left a bare `{}` in the output - the braces were taken for literal text while the assignment inside was still consumed. Such a block now counts as live, like `{a|b}` and `{...}==<v>`
  - VSmartPrompt: **Seedance dialogue braces no longer vanish** - after resolving, the node re-ran the resolver on its own output for LoRA cleanup even when no LoRA tag was present. That second pass had lost the `/# mode #/` directive and ate `{spoken line}` as a one-choice combination. The re-run now only happens when a LoRA tag was really stripped, and it keeps the mode
- v1.18.0
  - VSmartPrompt: **output modes** - a dropdown in the editor toolbar switches between `normal`, `H3 t2v`, `H3 r2v` and `Seedance 2.0`. The mode is stored as a comment directive inside the prompt (`/# mode: seedance #/`), so it travels with the workflow and reaches the backend. Without a directive nothing changes: a prompt resolves exactly as it did before
  - VSmartPrompt: in **Seedance** mode a `{...}` group only rolls when it carries a top-level `|` or a `::` weight, or when `==<name>` follows it. Every other pair stays literal text, so spoken lines like `{Hello, world}` reach the output unchanged. Literal braces are blue in the editor, rolling ones keep their usual frame colour
  - VSmartPrompt: in **H3** mode the speech and structure tags (`<d>`, `<i>`, `<breath>`, `<pause>`, `<scenetrans>`, `<cutoff>`, ...) are never read as variable references, even when a variable of that name exists, and are coloured as tags
  - The full VSmartPrompt syntax - combinations, wildcards, variables, switchers, comments - keeps working unchanged in every mode
- v1.17.0
