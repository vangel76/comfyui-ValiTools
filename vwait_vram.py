...
def _free_vram_gb(device_index: int) -> float | None:
    """Free VRAM as the driver reports it (so memory held by OTHER processes counts).
    Returns None when there is no CUDA device to look at."""
...
Free memory is read from the CUDA driver, so VRAM held by OTHER processes is
counted too. This node only waits - it never unloads or frees anything itself.
...
"""
    ...
    def main(self, min_free_gb, device_index, poll_seconds, timeout_seconds, on_timeout, any_in=None, unique_id=None):
        device_index = int(device_index)
        free_gb = _free_vram_gb(device_index)
